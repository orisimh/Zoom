	#include "StandardRoom.h"
//#include "Grade.h"
#include <string>
using namespace std;
#include <iostream>
#include <vector>


//nt Room::numroom =0;
StandardRoom::StandardRoom(): Room( ){

//	_roomid= numroom;
//	numroom++;
}

//
// void StandardRoom::addSingleLazyStudent(Student *st ){}
//
// void StandardRoom::addSingleStandartStudent(Student *st ){
//	 svec.push_back(st);
// }
//
// void StandardRoom::addSingleWorkStudent(Student *st ){
//
//}

 void StandardRoom:: addLazyStudent(vector<Student*> studentsvec ){

//	 int i =0;
//	 for (i = 0 ;i < studentsvec.size();i++){
//
//		 studentsvec[i]->addLazyStudent(this);
//		 studentsvec[i]->addWorkStudent(this);
//		 studentsvec[i]->addResponsibleStudent(this);
//		 //svec.push_back(st);
//	 }
 }
 void StandardRoom:: addStandartStudent(vector<Student*> studentsvec){

		unsigned long i = 0;
	//	vector<Student*> svs;
		for (i = 0 ;i < studentsvec.size();i++){

		  svec.push_back(studentsvec[i]);

		  studentsvec[i]->setRoom(this);

		  Dad->RemoveStudent(studentsvec[i]);
		  //studentsvec.erase(studentsvec.begin()+i );
 }
 }

 void StandardRoom:: addWorkStudent(vector<Student*> studentsvec){}

// void StandardRoom::removeStudent(){}
 //void LazyRoom::SplitRoom(){}

vector<Room*>  StandardRoom::SplitRoombyCharacter(){

	 vector<Room*> roomvec;

	 left = new WorkRoom();
	 left->setDad(this);

	  right = new LazyRoom();
	  right->setDad(this);



	 // cout<<"hello";
	 // left->addLazyStudent(svec);
	  right->addLazyStudent(svec);

	  left->addWorkStudent(svec);

	 // right->addWorkStudent(svec);
	  roomvec.push_back(left);
	 	  roomvec.push_back(right);
	  ifsplitted = true;

	  return roomvec;

  }
 vector<Room*>  StandardRoom::SplitRoombySize(){

	 	 vector<Room*> roomvec;
	  	  left = new StandardRoom();
	  	  left->setDad(this);

	  	  right = new StandardRoom();
	  	  right->setDad(this);

	  	 roomvec.push_back(left);
	  	 roomvec.push_back(right);
	  	  //left->addLazyStudent(svec);
	  	//lines;
	  	  std::size_t const half_size = svec.size() / 2;

	  	  cout<<half_size<<endl;

	  	  vector<Student*>split_l(svec.begin(), svec.begin() + half_size);
	   	  vector<Student*>split_r(svec.begin() + half_size, svec.end());

	      left->addStandartStudent(split_l);
	  	  right->addStandartStudent(split_r);
	  	  ifsplitted = true;

	  	  svec.clear();

	  	return roomvec;

  }


//   vector<Student*> StandardRoom::getStudents(){
//	   return svec;
//
//   }


//   void StandardRoom::setDad(Room *rom){
//
//   	   Dad = rom;
//
//      }

//    void StandardRoom:: removeRoom(){}

//     int  StandardRoom::getRoomid(){
//    	 return _roomid;
//     }

   StandardRoom::~StandardRoom()
{}

//   Room* StandardRoom::getDad(){
//
//   	return Dad;
//   }

//void Student::setRoom(int _room){}
//void Student::setMessage(string msg){}
/*
Student::Student(int id, const char* name):m_id(id),m_num(0)
{strcpy(m_name, name);
}


*/
